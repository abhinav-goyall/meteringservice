...
node 
{
   ...
   stage('Checkout')
   {
       ...
       sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/smartmeter-dev-apps/${MS_NAME}:smartmeter-test-apps --dry-run -o yaml >> Orchestration/deployment-smartmeter-test.yaml'
sh 'oc set image --local=true -f Orchestration/deployment.yaml ${MS_NAME}=${DOCKER_REGISTRY}/smartmeter-dev-apps/${MS_NAME}:smartmeter-dev-apps --dry-run -o yaml >> Orchestration/deployment-smartmeter-dev.yaml'
  ...
  if(env.SECURITY_TESTING == 'True')
  {
   ...
   {
       FAILED_STAGE=env.STAGE_NAME
       sh 'oc config use-context smartmeter-dev-apps'
       sh 'oc config view'
       try{
       sh 'oc new-build "${GIT_SOURCE_URL}" --strategy=docker -n=smartmeter-dev-apps'
       }catch(e){
       sh 'oc start-build meteringservice -n=smartmeter-dev-apps --wait=true'
       }
   ...
   stage('Tagging Image for Dev')
   {
         ...
         openshiftTag(namespace: 'smartmeter-dev-apps', srcStream: '$MS_NAME', srcTag: 'latest', destStream: '$MS_NAME', destTag: 'smartmeter-dev-apps')
   ...
   {   
       ...
       sh 'oc config use-context smartmeter-dev-apps'
       sh 'oc config view'
       sh 'oc apply -f Orchestration/deployment-smartmeter-dev.yaml -n=smartmeter-dev-apps'
       sh 'oc apply -f Orchestration/service.yaml -n=smartmeter-dev-apps'
       
   }
	
   stage('smartmeter-test - Tagging Image for Testing')
   {
   
   
        openshiftTag(namespace: 'smartmeter-dev-apps', srcStream: '$MS_NAME', srcTag: 'latest', destStream: '$MS_NAME', destTag: 'smartmeter-test-apps')
	...
	stage('smartmeter-test - Deploy Application')
	{
	
       FAILED_STAGE=env.STAGE_NAME
       sh 'oc config use-context smartmeter-test-apps'
       sh 'oc config view'	
	   sh 'oc apply -f Orchestration/deployment-smartmeter-test.yaml -n=smartmeter-test-apps'
    		   sh 'oc apply -f Orchestration/service.yaml -n=smartmeter-test-apps'
	}
